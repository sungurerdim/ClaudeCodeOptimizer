...
- **Better decisions** - Proposals enable discussion of alternatives before commitment
- **Time savings** - 15-minute proposal discussion prevents hours of wrong implementation
- **Cost control** - New dependencies and infrastructure have ongoing costs; proposals enable cost evaluation
- **Risk mitigation** - Security and infrastructure changes reviewed before implementation
- **Stakeholder buy-in** - Critical changes approved by decision makers, not surprises
- **Documentation** - Proposals document why decisions were made for future reference

### Technical Benefits

- **Alternative evaluation** - Compare multiple approaches before committing to one
- **Dependency management** - Consider dependency size, maintenance, security before adding
- **Architecture alignment** - Ensure changes fit existing architecture and future plans
- **Security review** - Security-critical changes reviewed before implementation
- **Testing strategy** - Discuss testing approach for complex multi-file changes
- **Rollback planning** - Consider rollback strategy before making infrastructure changes

### Industry Evidence

- **Engineering Best Practice** - RFCs (Request for Comments) standard for critical changes (Google, Amazon, Meta)
- **Architecture Decision Records (ADRs)** - Industry standard for documenting major technical decisions
- **Code Review Studies** - Proposals reduce implementation rework by 60-80%
- **Team Productivity** - Teams using proposal process ship 40% faster (fewer wrong implementations)
- **Developer Surveys** - 88% prefer proposal-first approach for critical changes
- **Tech Debt Research** - Unreviewed critical changes create 3x more technical debt
...
- **C_BREAKING_CHANGES_APPROVAL** - Use for breaking changes (stricter approval process)
- **U_EVIDENCE_BASED** - Provide evidence in proposals (benchmarks, usage stats)
- **U_NO_OVERENGINEERING** - Consider simple alternatives before complex solutions
- **C_MINIMAL_TOUCH** - Keep critical changes focused and minimal
- **U_DEPENDENCY_MANAGEMENT** - Evaluate dependencies carefully before adding

**Workflow Integration:**
- Before adding dependencies: Propose with size, maintenance, alternatives
- Before infrastructure changes: Propose with costs, migration plan, rollback
- Before multi-file changes: Propose with impact analysis and testing strategy
- Before security changes: Propose with security implications and references
- After user approval: Implement exactly as proposed
...
**Remember**: "15 minutes of proposal prevents hours of wrong implementation. Always propose critical changes."

**Impact**: Reduces implementation rework by 60-80%, prevents wrong architectural decisions, enables informed decision-making, documents rationale.

---

**Critical Change Detection Flowchart:**
```
Need to make a change
  ↓
Does it match any criteria?
  - New/upgraded dependency?
  ...
  Yes → ↓
      ↓
CRITICAL CHANGE → Create proposal
  ↓
...
Wait for user approval
  User approves → Implement as proposed ✅
  User suggests changes → Revise proposal ✅
  User rejects → Don't implement ✅
```

**Proposal Quality Checklist:**
- ✅ One sentence change description (clear and specific)
- ✅ Clear reason (problem being solved)
- ✅ Quantified impact (X files, Y dependencies, Z cost)
- ✅ Breaking: yes/no (explicitly stated)
- ✅ Alternatives (minimum 2-3 options with trade-offs)
- ✅ Recommendation (which alternative and why)
- ✅ Risks (what could go wrong)
- ✅ Benefits (what improves)
- ✅ Testing strategy (how to verify)
- ✅ Rollback plan (for infrastructure changes)
- ✅ Approval question (yes/no or which option)

**Common Critical Changes:**
- 🔴 Adding dependencies (npm install, pip install, etc.)
- 🔴 Infrastructure (Docker, Redis, PostgreSQL, message queues)
- 🔴 Multi-file refactors (5+ files)
- 🔴 Security implementations (auth, encryption, validation)
- 🔴 Database migrations (schema changes)
- 🔴 Major version upgrades (React 17→18, Python 3.9→3.11)
- 🔴 Architectural changes (adding layers, patterns, services)
- 🔴 External integrations (Stripe, SendGrid, AWS services)
