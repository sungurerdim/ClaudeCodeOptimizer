...
function getGitInfo() {
  ...
  const unstagedCount = execCommand('git diff --name-only | wc -l');
  const stagedCount = execCommand('git diff --cached --name-only | wc -l');
  const untrackedCount = execCommand('git ls-files --others --exclude-standard | wc -l');

  const totalChanges = parseInt(unstagedCount || 0) + parseInt(stagedCount || 0) + parseInt(untrackedCount || 0);
  ...
  return {
    ...
    unstagedFiles: parseInt(unstagedCount || 0),
...
function formatMetricsForStatusline(inputData, metrics, telemetryData) {
  ...
  const linesAdded = inputData.cost?.total_lines_added || 0;
  const linesRemoved = inputData.cost?.total_lines_removed || 0;
  ...
  // Build status lines based on config (OPTIMIZED UX - 2 LINE LAYOUT)
  const lines = [];

  // === LINE 1: CC Version + Model + User@hostname + Directory ===
  const line1Parts = [];

  // Claude Code version (first, dim gray for subtle display)
  ...
  if (claudeVersion && statuslineConfig.show_claude_version !== false) {
    ...
    if (versionMatch) {
      const versionDisplay = colorize(`CC ${versionMatch[1]}`, 'gray', colorEnabled);
      line1Parts.push(versionDisplay);
    }
  }

  // Model name (bold + bright for extra visibility) - RIGHT AFTER CC VERSION
  ...
  if (statuslineConfig.show_directory !== false) {
    line1Parts.push(colorize(fullPath, 'blue', colorEnabled));
  }

  lines.push(line1Parts.join(' | '));

  // === LINE 2: Git Repo:Branch + Git Status (ALWAYS VISIBLE) + Lines Changed ===
  const line2Parts = [];
  ...
  if (metrics.git && statuslineConfig.show_git_info !== false) {
    ...
    // Git status details (ALWAYS show, even if 0)
    const gitStatusParts = [];

    const stagedCount = metrics.git.stagedFiles || 0;
    const unstagedCount = metrics.git.unstagedFiles || 0;
    const unpushedCount = metrics.git.unpushedCommits || 0;

    const stagedColor = stagedCount > 0 ? 'green' : 'gray';
    const unstagedColor = unstagedCount > 0 ? 'yellow' : 'gray';
    const unpushedColor = unpushedCount > 0 ? 'cyan' : 'gray';

    gitStatusParts.push(colorize(`Staged: ${stagedCount}`, stagedColor, colorEnabled));
    gitStatusParts.push(colorize(`Unstaged: ${unstagedCount}`, unstagedColor, colorEnabled));
    gitStatusParts.push(colorize(`Unpushed: ${unpushedCount}`, unpushedColor, colorEnabled));

    line2Parts.push(gitStatusParts.join(', '));
  }

  // Lines changed (only show if > 0)
  if (linesAdded > 0 || linesRemoved > 0) {
    const addDisplay = colorize(`+${linesAdded}`, 'green', colorEnabled);
    const removeDisplay = colorize(`-${linesRemoved}`, 'red', colorEnabled);
    line2Parts.push(`Lines: ${addDisplay}/${removeDisplay}`);
  }

  // Docker status (inline if enabled)
  if (metrics.docker && statuslineConfig.show_docker_status === true) {
    line2Parts.push(`Docker: ${metrics.docker.running}/${metrics.docker.total}`);
  }
  ...
  if (metrics.cco && metrics.cco.enabled) {
    ...
    if (ccoParts.length > 0) {
      line2Parts.push(`CCO: ${ccoParts.join(' ')}`);
    }
  }
