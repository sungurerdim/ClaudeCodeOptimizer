...
allowed-tools: Read(*), Write(*), Edit(*), Grep(*), Glob(*), Bash(git:*), Bash(cco-install:*), Bash(sed:*), Bash(find:*), Bash(wc:*), Task(*), TodoWrite
---
...
|------|----------|--------------|
| `cco-install` | `~/.claude/` (global) | Commands, Agents, Rules to `rules/cco/` |
| `cco-config` | `./` (project local) | Project context + rules + statusline + permissions |
| `cco-remove` | Both | Global + local cleanup |

### Global vs Local

**cco-install installs global files:**
...
**cco-config creates local project files:**
...
Statusline and permissions are installed via `cco-install --local` command:

```bash
# Install statusline (mode: full or minimal)
cco-install --local . --statusline full

# Install permissions (level: safe, balanced, permissive, or full)
cco-install --local . --permissions balanced
...
**IMPORTANT:** cco-config uses `cco-install --local` via Bash tool to install statusline and permissions. This ensures templates are always read from the installed package, not generated by AI.

Global `~/.claude/` files are never modified by cco-config.

## Usage

```bash
/cco-config              # Show status, then choose: Configure / Remove / Export
```
...
- **cco-config NEVER modifies global ~/.claude/ files**
...
> **Note:** Claude Code does not define fixed tiers. These are **CCO-recommended values** based on project complexity. Official docs only specify: MAX_MCP_OUTPUT_TOKENS default=25000, warning at 10000.
>
> **v2.0.67 Change:** Thinking mode toggle has moved to `/config` command (Tab to toggle, sticky across sessions). Thinking is now **enabled by default for Opus 4.5**. MAX_THINKING_TOKENS env variable still controls the token budget when thinking is enabled.
...
├─────┼───────────────┼──────────────────────┼──────┼─────────────────────┼───────────────────┤
║  1  │ Purpose       │ {purpose}            │ ●●●  │ {source}            │ → context         ║
║  2  │ Stack         │ {stack}              │ ●●●  │ {source}            │ → tools, context  ║
║  3  │ Type          │ {type}               │ ●●●  │ {source}            │ → {type} rules    ║
║  4  │ License       │ {license}            │ ●●●  │ {source}            │ → context         ║
╠═════╪═══════════════╪══════════════════════╪══════╪═════════════════════╪═══════════════════╣
║     │ INFRASTRUCTURE                                                                         ║
├─────┼───────────────┼──────────────────────┼──────┼─────────────────────┼───────────────────┤
║  5  │ CI/CD         │ {ci_cd}              │ {c}  │ {source}            │ → Ops rules       ║
║  6  │ Container     │ {container}          │ {c}  │ {source}            │ {action}          ║
║  7  │ K8s           │ {k8s}                │ {c}  │ {source}            │ {action}          ║
╠═════╪═══════════════╪══════════════════════╪══════╪═════════════════════╪═══════════════════╣
║     │ QUALITY & SECURITY                                                                     ║
├─────┼───────────────┼──────────────────────┼──────┼─────────────────────┼───────────────────┤
║  8  │ Test Framework│ {test_framework}     │ {c}  │ {source}            │ → testing tier    ║
║  9  │ Coverage      │ {coverage}%          │ {c}  │ {source}            │ → context         ║
║ 10  │ Linting       │ {linters}            │ {c}  │ {source}            │ → tools           ║
║ 11  │ Pre-commit    │ {precommit}          │ {c}  │ {source}            │ → hooks: {status} ║
║ 12  │ Security Scan │ {sec_scan}           │ {c}  │ {source}            │ {action}          ║
║ 13  │ Secrets Risk  │ {secrets}            │ {c}  │ {source}            │ {action}          ║
╠═════╪═══════════════╪══════════════════════╪══════╪═════════════════════╪═══════════════════╣
║     │ AI PERFORMANCE (auto-calculated → ./.claude/settings.json)                             ║
├─────┼───────────────┼──────────────────────┼──────┼─────────────────────┼───────────────────┤
║ 14  │ Thinking      │ {thinking} ({tier})  │ ●●●  │ complexity: {score} │ → env             ║
║ 15  │ MCP Output    │ {mcp} ({tier})       │ ●●●  │ files: {count}      │ → env             ║
║ 16  │ Caching       │ {caching}            │ ●●●  │ recommended         │ → env             ║
╠═════╪═══════════════╪══════════════════════╪══════╪═════════════════════╪═══════════════════╣
║     │ CONTEXT (smart defaults applied, editable)                                             ║
├─────┼───────────────┼──────────────────────┼──────┼─────────────────────┼───────────────────┤
║ 17  │ Team          │ {team}               │ {c}  │ {source}            │ → guidelines      ║
║ 18  │ Scale         │ {scale}              │ {c}  │ {source}            │ → Caching     ⚠   ║
║ 19  │ Data          │ {data}               │ {c}  │ {source}            │ → guidelines      ║
║ 20  │ Compliance    │ {compliance}         │ {c}  │ {source}            │ {action}      ⚠   ║
║ 21  │ Architecture  │ {architecture}       │ {c}  │ {source}            │ → guidelines      ║
║ 22  │ Testing       │ {testing}            │ {c}  │ {source}            │ → Testing stds    ║
║ 23  │ Maturity      │ {maturity}           │ {c}  │ {source}            │ → guidelines      ║
║ 24  │ Breaking      │ {breaking}           │ {c}  │ {source}            │ → guidelines      ║
║ 25  │ Priority      │ {priority}           │ {c}  │ {source}            │ → guidelines  ⚠   ║
╠══════════════════════════════════════════════════════════════════════════════════════════════╣
║ ⚠ LOW CONFIDENCE: Items with ●○○ may need review                                            ║
╠══════════════════════════════════════════════════════════════════════════════════════════════╣
║ RULES TRIGGERED (each evaluated individually for relevance)                                  ║
├──────────────────────────────────────────────────────────────────────────────────────────────┤
║ {triggered_categories} = {N} project-specific (counted after evaluation)                     ║
╠══════════════════════════════════════════════════════════════════════════════════════════════╣
...
│ ☐ {N}: Team     Currently: {value}                              │
│ ☐ {N}: Scale    Currently: {value}                              │
│ ☐ {N}: Data     Currently: {value}                              │
└─────────────────────────────────────────────────────────────────┘
```
...
| Statusline | `./.claude/statusline.js` + `./.claude/settings.json` | `cco-install --local . --statusline {mode}` |
| Permissions | `./.claude/settings.json` | `cco-install --local . --permissions {level}` |
...
Use Bash tool to call `cco-install --local`:
...
cco-install --local . --statusline {mode}

# If user selected permissions only
cco-install --local . --permissions {level}
```
...
**IMPORTANT:** cco-config NEVER modifies global `~/.claude/` files. All settings are project-local.
...
- Any check fails → Show `Statusline: Broken (run /cco-config to fix)`
...
cco-config generates files in `./.claude/rules/cco/` using path-specific YAML frontmatter:
...
║ Next: /cco-status to verify | /cco-optimize to check                              ║
╚════════════════════════════════════════════════════════════════════════════════╝
```
...
```
║ Rules          │ {before}          │ removed           │ user requested    ║
║ AI Performance │ {before} tokens   │ removed           │ user requested    ║
║ Statusline     │ {before}          │ removed           │ user requested    ║
║ Permissions    │ {before} ({N})    │ removed           │ user requested    ║
```
...
║ Setting        │ Before            │ After             │ Reason            ║
├────────────────┼───────────────────┼───────────────────┼───────────────────┤
║ Rules          │ {before}          │ {after}           │ {reason}          ║
║ AI Performance │ {before} tokens   │ {after}           │ {reason}          ║
║ Statusline     │ {before}          │ {after}           │ {reason}          ║
║ Permissions    │ {before} ({N})    │ {after}           │ {reason}          ║
```
...
**Prerequisites:**
- `~/.claude/rules/cco/` MUST exist with core.md, ai.md (run `cco-install` first if missing)
- `./.claude/rules/cco/` is optional (export will include global rules only)

**Export process:**
1. Check `~/.claude/rules/cco/core.md` exists → error if missing: "Run cco-install first"
...
9. **Never modify global** - cco-config may READ `~/.claude/rules/cco/` for counting, but NEVER write/modify any file in `~/.claude/`
