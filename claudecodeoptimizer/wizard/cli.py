#!/usr/bin/env python3
"""
CCO Interactive Wizard - Main CLI
...
class CCOWizard:
    ...
    def __init__(self, project_root: str, mode: str = "quick", dry_run: bool = False) -> None:
    ...
    def _show_welcome(self) -> None:
        ...
        if self.dry_run:
            print_header(
                f"CCO {__version__} Interactive Wizard (DRY RUN)",
                "No files will be modified - preview mode only",
            )
        else:
            print_header(
                f"CCO {__version__} Interactive Wizard",
    ...
    def _run_apply(self) -> bool:
        ...
        try:
            ...
            # Generate CLAUDE.md
            print_info("Generating CLAUDE.md...", indent=2)
            self._generate_claude_md()
            print_success("CLAUDE.md created", indent=2)
            print()
...
Generated by CCO {__version__} on {datetime.now().strftime("%Y-%m-%d")}
...
"""
    ...
    def _generate_claude_md(self) -> None:
        ...
        # Generate CLAUDE.md using preferences
        generator = ClaudeMdGenerator(self.preferences.dict())
        generator.generate(claude_md_path)
    ...
    def _show_completion(self, duration: float) -> None:
        ...
        # Count actual principles from selection
        principles_count = len(self.selected_principle_ids) if self.selected_principle_ids else 0

        display_completion_summary(
            commands_installed=len(self.selected_commands),
            principles_configured=principles_count,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=f"CCO {__version__} Interactive Wizard",
